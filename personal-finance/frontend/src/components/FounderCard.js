...
export default function FounderCard({ name, role, socials, photoClass, photoUrl, onClick }) {
  return (
    <div className="card" onClick={onClick} style={{ cursor: 'pointer' }}>
      {photoUrl ? (
        <img src={photoUrl} alt={name} className={`card-photo founder-photo-img ${photoClass}`} />
      ) : (
        <div className={`card-photo ${photoClass}`}></div>
      )}
      <div className="card-title">{name}<br/>
        <span>{role}</span>
      </div>
      <div className="card-socials">
        <a className="card-socials-btn facebook" href={socials.facebook} target="_blank" rel="noopener noreferrer">
          <svg className="icon" xmlns="http://www.w3.org/2000/svg" width="24" viewBox="0 0 24 24" height="24"><path d="M16.75,9H13.5V7a1,1,0,0,1,1-1h2V3H14a4,4,0,0,0-4,4V9H8v3h2v9h3.5V12H16Z"></path></svg>
        </a>
        <a className="card-socials-btn github" href={socials.github} target="_blank" rel="noopener noreferrer">
        ...
        <a className="card-socials-btn linkedin" href={socials.linkedin} target="_blank" rel="noopener noreferrer">
